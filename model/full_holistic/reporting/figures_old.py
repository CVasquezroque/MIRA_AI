...
def write_shap_group_figures(group_frame: pd.DataFrame, output_dir: Path, *, focus_model: str | None = None) -> list[dict[str, str]]:
    if group_frame.empty or not {"model", "group", "feature", "mean_abs_shap"}.issubset(group_frame.columns):
        return []
    plt, sns = setup_matplotlib()
    ...
    if frame.empty:
        return []
    top_features = frame.groupby("feature")["mean_abs_shap"].mean().sort_values(ascending=False).head(12).index
    pivot = frame[frame["feature"].isin(top_features)].pivot_table(index="group", columns="feature", values="mean_abs_shap", aggfunc="mean")
    if pivot.empty:
        return []
    ...
    if sns:
        ...
        ax.imshow(pivot.fillna(0), aspect="auto")
    ax.set_title("SHAP Contribution Patterns By Housing Status")
    ax.set_xlabel("Feature")
    ax.set_ylabel("Housing status")
    path = save_figure(fig, figure_dir(output_dir), "shap_group_contribution_heatmap.png")
    plt.close(fig)
    return [{"title": "SHAP contribution patterns by group", "path": path, "caption": "Mean absolute SHAP values by housing status group."}]
...
def _shap_report_figures(results_dir: Path, data: dict[str, object], output_dir: Path, focus_model: str | None) -> list[dict[str, str]]:
    ...
    if generated:
        group_path = _existing_figure(results_dir, "shap", "shap_group_contribution_heatmap.png")
        if group_path:
            generated.append({"title": "SHAP contribution patterns by group", "path": group_path, "caption": "Mean absolute SHAP values by housing status group."})
        return generated
